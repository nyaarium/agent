...
docker exec -d -u vscode "$CONTAINER_ID" \
	tmux new-session -d -s "$TMUX_SESSION" "source ~/.bashrc; cd /workspace/${PROJECT_NAME}; claude --name '${PROJECT_NAME}' --rc --dangerously-skip-permissions --dangerously-load-development-channels plugin:agent-team-bridge@agent-team-bridge; exec bash"
